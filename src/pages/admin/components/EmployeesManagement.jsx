...
const emptyForm = {
  ...
  jobTitle: '', shift: 'صباحي', nationalId: '', address: '', department: ''
};
    ...
    <div className="p-6 space-y-6 fade-in font-cairo">
      ...
      {isAdmin && (
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
          ...
          <form onSubmit={createEmployee} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <input value={form.department} onChange={(e) => set('department', e.target.value)} className="px-3 py-2.5 rounded-xl border bg-slate-50 focus:bg-white focus:border-purple-400 outline-none font-cairo" placeholder="القسم" />
      ...
      <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100">
        <table className="hospital-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={isFinance ? "7" : "6"} className="text-center py-8 text-slate-400 animate-pulse">جاري التحميل...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={isFinance ? "7" : "6"} className="text-center py-8 text-slate-400">لا توجد نتائج مطابقة للبحث</td></tr>
              ...
              <tr key={employee.id}>
                ...
                <td className="text-slate-500 text-sm">{employee.department || '—'}</td>
